...
├──────────────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                              │
│   CLIENT LAYER                    SCHEDULER SERVER                    STORAGE LAYER          │
│                                                                                              │
│   ┌──────────────┐               ┌────────────────────────┐          ┌──────────────────┐    │
│   │   gRPC       │               │                        │          │                  │    │
│   │   Clients    │──┐            │    gRPC API Server     │          │   PostgreSQL     │    │
│   │              │  │  :50051     │  ┌──────────────────┐ │          │   :5432          │    │
│   │  - Submit    │  ├────────────▶│  │  Health Checks  │ │          │                  │    │
│   │  - Query     │  │            │  │  Liveness       │ │    ┌────▶│  Task Storage     │    │
│   │  - Stream    │  │            │  │  Readiness      │ │    │     │  FOR UPDATE       │    │
│   │  - Cancel    │  │            │  └──────────────────┘ │    │     │  SKIP LOCKED      │    │
│   └──────────────┘  │            │                        │    │     │                  │    │
│                     │            │  ┌──────────────────┐ │    │     └──────────────────┘    │
│   ┌──────────────┐  │            │  │    Scheduler     │ │    │                             │
...
│   │                              DATA FLOW                                                  ││
│   │                                                                                         ││
│   │   Client ──▶ gRPC ──▶ Scheduler ──▶ DAG Resolve ──▶ Redis Queue ──▶ Workers ──▶ PG     ││
│   │      │                   │                                                       │      ││
│   │      │                   ├──▶ Status Query ──▶ PostgreSQL ◀──────────────────────┘      ││
│   │      │                   │                                                               ││
│   │      └──▶ Stream Events ◀─── Worker Events ──▶ Prometheus ──▶ Metrics                    ││
│   └─────────────────────────────────────────────────────────────────────────────────────────┘│
│                                                                                              │
└──────────────────────────────────────────────────────────────────────────────────────────────┘
```
